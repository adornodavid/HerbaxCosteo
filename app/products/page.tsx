...
import { obtenerProductos, estatusActivoProducto, obtenerProductosAvanzado, listadopresentacion, listadotipocomision, listadofrecuencia, listadocodigomaestro, listadocodigo, listadocodigointerno, listadonombrematerial, listadocodigomaterial, listadodetallematerial, listadoespecificacionesmaterial, listadonombresformulas, listadocodigosformulas, listadoespecificacionesformulas, listadonombresmateriaspri, listadocodigosmateriaspri, listadofamiliasmateriaspri, listadoespecificacionesmateriaspri, listadopresentacionesmateriaspri, listadoformula, listadomedidaformula, listadofamiliamaterialempaque, listadopais, listadomedidaempaque, listadocolorempaque, listadofamiliamateriaprima, listadopresentacionmateriaprima, FiltrosAvanzados } from "@/app/actions/products"
...
export default function ProductosPage() {
  ...
  useEffect(() => {
    const cargarOpciones = async () => {
      try {
        console.log('[v0] Cargando filtros avanzados desde FiltrosAvanzados()...')

        // Llamar a la función FiltrosAvanzados que consulta filtrosavanzadosproductos()
        const result = await FiltrosAvanzados()

        if (!result.success || !result.data || result.data.length === 0) {
          console.error('[v0] Error al cargar filtros avanzados:', result.error)
          setModalError({
            Titulo: "Error al cargar opciones de filtros",
            Mensaje: `Hubo un error al cargar las opciones para los filtros avanzados: ${result.error}`,
          })
          setShowModalError(true)
          return
        }

        // El resultado viene como un array con un solo objeto que contiene todas las columnas
        const filtrosData = result.data[0]

        console.log('[v0] Filtros avanzados recibidos:', filtrosData)

        // Mapear productos_codigomaestro (son strings simples para el autocomplete)
        if (filtrosData.productos_codigomaestro && Array.isArray(filtrosData.productos_codigomaestro)) {
          const codigosMaestrosArray = filtrosData.productos_codigomaestro.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setCodigosMaestros(codigosMaestrosArray)
          setCodigoMaestroFiltrado(codigosMaestrosArray)
        }

        // Mapear productos_codigo (son strings simples para el autocomplete)
        if (filtrosData.productos_codigo && Array.isArray(filtrosData.productos_codigo)) {
          const codigosArray = filtrosData.productos_codigo.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setCodigos(codigosArray)
          setCodigoFiltrado(codigosArray)
        }

        // Mapear productos_codigointerno (son strings simples para el autocomplete)
        if (filtrosData.productos_codigointerno && Array.isArray(filtrosData.productos_codigointerno)) {
          const codigosInternosArray = filtrosData.productos_codigointerno.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setCodigosInternos(codigosInternosArray)
          setCodigoInternoFiltrado(codigosInternosArray)
        }

        // Mapear productos_categoria -> tiposComisionesOptions (necesita formato {value, text})
        if (filtrosData.productos_categoria && Array.isArray(filtrosData.productos_categoria)) {
          const tiposComisionTransformados = filtrosData.productos_categoria.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setTiposComisionesOptions(tiposComisionTransformados)
        }

        // Mapear productos_envase -> envasesOptions (necesita formato {value, text})
        if (filtrosData.productos_envase && Array.isArray(filtrosData.productos_envase)) {
          const envasesTransformados = filtrosData.productos_envase.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setEnvasesOptions([{ value: "-1", text: "Todos" }, ...envasesTransformados])
        }

        // Mapear productos_presentacion -> presentaciones (necesita formato {value, text})
        if (filtrosData.productos_presentacion && Array.isArray(filtrosData.productos_presentacion)) {
          const presentacionesTransformadas = filtrosData.productos_presentacion.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setPresentaciones(presentacionesTransformadas)
        }

        // Mapear productos_subforma -> formasFarmaceuticasOptions (necesita formato {value, text})
        if (filtrosData.productos_subforma && Array.isArray(filtrosData.productos_subforma)) {
          const subformasTransformadas = filtrosData.productos_subforma.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setFormasFarmaceuticasOptions([{ value: "-1", text: "Todos" }, ...subformasTransformadas])
        }

        // Mapear sistemas -> objetivosOptions (necesita formato {value, text})
        if (filtrosData.sistemas && Array.isArray(filtrosData.sistemas)) {
          const sistemasTransformados = filtrosData.sistemas.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setObjetivosOptions([{ value: "-1", text: "Todos" }, ...sistemasTransformados])
        }

        // Mapear productos_frecuencia -> frecuencias (son strings simples)
        if (filtrosData.productos_frecuencia && Array.isArray(filtrosData.productos_frecuencia)) {
          const frecuenciasArray = filtrosData.productos_frecuencia.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setFrecuencias(frecuenciasArray)
        }

        // Mapear formulas_nombre -> nombresFormulas (son strings simples para el autocomplete)
        if (filtrosData.formulas_nombre && Array.isArray(filtrosData.formulas_nombre)) {
          const nombresFormulasArray = filtrosData.formulas_nombre.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setNombresFormulas(nombresFormulasArray)
          setNombreFormulaFiltrado(nombresFormulasArray)
        }

        // Mapear formulas_codigo -> codigosFormulas (son strings simples para el autocomplete)
        if (filtrosData.formulas_codigo && Array.isArray(filtrosData.formulas_codigo)) {
          const codigosFormulasArray = filtrosData.formulas_codigo.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setCodigosFormulas(codigosFormulasArray)
          setCodigoFormulaFiltrado(codigosFormulasArray)
        }

        // Mapear formulas_especificaciones -> especificacionesFormulas (son strings simples para el autocomplete)
        if (filtrosData.formulas_especificaciones && Array.isArray(filtrosData.formulas_especificaciones)) {
          const especificacionesFormulasArray = filtrosData.formulas_especificaciones.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setEspecificacionesFormulas(especificacionesFormulasArray)
          setEspecificacionesFormulaFiltrado(especificacionesFormulasArray)
        }

        // Mapear formulas_formula -> formulasDropdown (necesita formato {value, text})
        if (filtrosData.formulas_formula && Array.isArray(filtrosData.formulas_formula)) {
          const formulasTransformadas = filtrosData.formulas_formula.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setFormulasDropdown(formulasTransformadas)
        }

        // Mapear formulas_medida -> medidasFormula (necesita formato {value, text})
        if (filtrosData.formulas_medida && Array.isArray(filtrosData.formulas_medida)) {
          const medidasTransformadas = filtrosData.formulas_medida.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setMedidasFormula(medidasTransformadas)
        }

        // Mapear materialesetiquetado_nombre -> nombresMateriales (son strings simples para el autocomplete)
        if (filtrosData.materialesetiquetado_nombre && Array.isArray(filtrosData.materialesetiquetado_nombre)) {
          const nombresMaterialesArray = filtrosData.materialesetiquetado_nombre.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setNombresMateriales(nombresMaterialesArray)
          setNombreMaterialFiltrado(nombresMaterialesArray)
        }

        // Mapear materialesetiquetado_codigo -> codigosMateriales (son strings simples para el autocomplete)
        if (filtrosData.materialesetiquetado_codigo && Array.isArray(filtrosData.materialesetiquetado_codigo)) {
          const codigosMaterialesArray = filtrosData.materialesetiquetado_codigo.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setCodigosMateriales(codigosMaterialesArray)
          setCodigoMaterialFiltrado(codigosMaterialesArray)
        }

        // Mapear materialesetiquetado_familia -> familiasEmpaque (necesita formato {value, text})
        if (filtrosData.materialesetiquetado_familia && Array.isArray(filtrosData.materialesetiquetado_familia)) {
          const familiasTransformadas = filtrosData.materialesetiquetado_familia.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setFamiliasEmpaque(familiasTransformadas)
        }

        // Mapear materialesetiquetado_detalle -> detallesMateriales (son strings simples para el autocomplete)
        if (filtrosData.materialesetiquetado_detalle && Array.isArray(filtrosData.materialesetiquetado_detalle)) {
          const detallesMaterialesArray = filtrosData.materialesetiquetado_detalle.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setDetallesMateriales(detallesMaterialesArray)
          setDetalleMaterialFiltrado(detallesMaterialesArray)
        }

        // Mapear materialesetiquetado_especificaciones -> especificacionesMateriales (son strings simples para el autocomplete)
        if (filtrosData.materialesetiquetado_especificaciones && Array.isArray(filtrosData.materialesetiquetado_especificaciones)) {
          const especificacionesMaterialesArray = filtrosData.materialesetiquetado_especificaciones.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setEspecificacionesMateriales(especificacionesMaterialesArray)
          setEspecificacionesMaterialFiltrado(especificacionesMaterialesArray)
        }

        // Mapear materialesetiquetado_pais -> paises (necesita formato {value, text})
        console.log('[v0] materialesetiquetado_pais RAW:', filtrosData.materialesetiquetado_pais)
        if (filtrosData.materialesetiquetado_pais && Array.isArray(filtrosData.materialesetiquetado_pais)) {
          const paisesTransformados = filtrosData.materialesetiquetado_pais.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          console.log('[v0] paises TRANSFORMADOS:', paisesTransformados)
          setPaises(paisesTransformados)
        }

        // Mapear materialesetiquetado_medida -> medidasEmpaque (necesita formato {value, text})
        console.log('[v0] materialesetiquetado_medida RAW:', filtrosData.materialesetiquetado_medida)
        if (filtrosData.materialesetiquetado_medida && Array.isArray(filtrosData.materialesetiquetado_medida)) {
          const medidasTransformadas = filtrosData.materialesetiquetado_medida.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          console.log('[v0] medidasEmpaque TRANSFORMADAS:', medidasTransformadas)
          setMedidasEmpaque(medidasTransformadas)
        }

        // Mapear materialesetiquetado_color -> coloresEmpaque (necesita formato {value, text})
        console.log('[v0] materialesetiquetado_color RAW:', filtrosData.materialesetiquetado_color)
        if (filtrosData.materialesetiquetado_color && Array.isArray(filtrosData.materialesetiquetado_color)) {
          const coloresTransformados = filtrosData.materialesetiquetado_color.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          console.log('[v0] coloresEmpaque TRANSFORMADOS:', coloresTransformados)
          setColoresEmpaque(coloresTransformados)
        }

        // Mapear materiasprima_nombre -> nombresMateriaPrima (son strings simples para el autocomplete)
        if (filtrosData.materiasprima_nombre && Array.isArray(filtrosData.materiasprima_nombre)) {
          const nombresMateriaPrimaArray = filtrosData.materiasprima_nombre.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setNombresMateriaPrima(nombresMateriaPrimaArray)
          setNombreMateriaPrimaFiltrado(nombresMateriaPrimaArray)
        }

        // Mapear materiasprima_codigo -> codigosMateriaPrima (son strings simples para el autocomplete)
        if (filtrosData.materiasprima_codigo && Array.isArray(filtrosData.materiasprima_codigo)) {
          const codigosMateriaPrimaArray = filtrosData.materiasprima_codigo.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setCodigosMateriaPrima(codigosMateriaPrimaArray)
          setCodigoMateriaPrimaFiltrado(codigosMateriaPrimaArray)
        }

        // Mapear materiasprima_especificaciones -> especificacionesMateriaPrima (son strings simples para el autocomplete)
        if (filtrosData.materiasprima_especificaciones && Array.isArray(filtrosData.materiasprima_especificaciones)) {
          const especificacionesMateriaPrimaArray = filtrosData.materiasprima_especificaciones.map((item: any) => 
            typeof item === 'string' ? item : item.value || item.text || item
          )
          setEspecificacionesMateriaPrima(especificacionesMateriaPrimaArray)
          setEspecificacionesMateriaPrimaFiltrado(especificacionesMateriaPrimaArray)
        }

        // Mapear materiasprima_familia -> familiasMateriaPrima (necesita formato {value, text})
        if (filtrosData.materiasprima_familia && Array.isArray(filtrosData.materiasprima_familia)) {
          const familiasTransformadas = filtrosData.materiasprima_familia.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setFamiliasMateriaPrima(familiasTransformadas)
        }

        // Mapear materiasprima_presentacion -> presentacionesMateriaPrima (necesita formato {value, text})
        if (filtrosData.materiasprima_presentacion && Array.isArray(filtrosData.materiasprima_presentacion)) {
          const presentacionesTransformadas = filtrosData.materiasprima_presentacion.map((item: any) => {
            if (typeof item === 'object' && (item.value || item.text)) {
              return { value: String(item.id || item.value), text: String(item.value || item.text) }
            }
            return { value: String(item), text: String(item) }
          })
          setPresentacionesMateriaPrima(presentacionesTransformadas)
        }

        console.log('[v0] Filtros avanzados cargados exitosamente desde FiltrosAvanzados()')
      } catch (error) {
        console.error('[v0] Error loading dropdown options:', error)
        setModalError({
          Titulo: "Error al cargar opciones de filtros",
                          ...
                          <Select value={filtroPresentacionMateriaPrima} onValueChange={setFiltroPresentacionMateriaPrima}>
                            ...
                            <SelectContent>
                              ...
                              {presentacionesMateriaPrima.map((presentacion) => (
                                <SelectItem key={presentacion.value} value={presentacion.value}>
                                  {presentacion.text}
                                </SelectItem>
                              ))}
                            </SelectContent>
