...
export async function listaDesplegableFormulasBuscar(buscar: string, clienteId?: number): Promise<{ success: boolean; data: ddlItem[] }> {
  try {
    let query = supabase.from("formulas").select("id, codigo, nombre, clienteid").eq("activo", true)
    ...
    // Apply filter by cliente if provided
