...
export default function EditarFormulaPage() {
  ...
  useEffect(() => {
    const loadFormulaData = async () => {
      if (!formulaId) {
        setErrorMessage("ID de fórmula no proporcionado")
      ...
      try {
        setIsLoading(true)
        const result = await obtenerFormulaCompleta(Number(formulaId))

        if (result.error || !result.data) {
          setErrorMessage("Error al cargar los datos de la fórmula")
          setShowErrorModal(true)
          return
        }
        ...
        if (formula.ingredientesxformula?.[0]?.ingredientes?.clienteid) {
          setSelectedClienteId(formula.ingredientesxformula[0].ingredientes.clienteid.toString())
        }
      } catch (error) {
        console.error("Error loading formula:", error)
        setErrorMessage("Error inesperado al cargar la fórmula")
        setShowErrorModal(true)
      } finally {
        setIsLoading(false)
      }
  ...
  useEffect(() => {
    const loadIngredientesFormula = async () => {
      if (formulaId) {
        try {
          const result = await obtenerIngredientesFormula(Number(formulaId))
          if (result.data) {
  ...
  useEffect(() => {
    const total = ingredientesAgregados.reduce((sum, ingrediente) => {
      return sum + (Number.parseFloat(ingrediente.ingredientecostoparcial.toString()) || 0)
    }, 0)
    setCostoTotal(total)
  ...
  const handleIngredienteSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIngredienteSearchTerm(value)
    setSelIngredienteId("")
  }

  const handleSelectIngredienteFromDropdown = (ingrediente: Ingrediente) => {
    setIngredienteSearchTerm(`${ingrediente.codigo} - ${ingrediente.nombre}`)
    setSelIngredienteId(ingrediente.id.toString())
    setShowIngredienteDropdown(false)
  }

  const handleAgregarIngrediente = async () => {
    if (!selIngredienteId || !selIngredienteCantidad || !formulaId) {
      setErrorMessage("Por favor completa todos los campos del ingrediente")
      setShowErrorModal(true)
      return
    }
    ...
    try {
      ...
      const selectedIngrediente = ingredientes.find((i) => i.id.toString() === selIngredienteId)
      ...
      const result = await crearFormulaEtapa2(
        ...
        selectedIngrediente.costo,
      )

      if (result.success) {
        // Recargar ingredientes
        const ingredientesResult = await obtenerIngredientesFormula(Number(formulaId))
        ...
        setErrorMessage(result.error || "Error al agregar ingrediente")
        setShowErrorModal(true)
      }
    } catch (error) {
  ...
  const handleNextStep = async () => {
    if (currentStep === 1) {
      ...
      setIsLoading(true)
      try {
        const result = await actualizarFormulaEtapa1(Number(formulaId), {
        ...
        if (result.success) {
          ...
          setErrorMessage(result.error || "Error al actualizar la fórmula")
          setShowErrorModal(true)
        }
      } catch (error) {
        setErrorMessage("Error inesperado al actualizar la fórmula")
        setShowErrorModal(true)
      } finally {
        setIsLoading(false)
      }
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-br from-slate-50 to-slate-100/50 backdrop-blur-sm border border-slate-200/60 rounded-xs p-6 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-4">
          ...
          <h3 className="col-span-2 text-lg font-medium text-slate-800 mb-4">Detalles de Porción</h3>
        </div>
                    ...
                    {ingredientesAgregados.map((i) => (
                      <TableRow key={i.id}>
                        ...
                        <TableCell className="text-right">
                          ${Number.parseFloat(i.ingredientecostoparcial.toString()).toFixed(2)}
                        </TableCell>
                        <TableCell>
                          <Button
                            variant="destructive"
          ...
          <div className="bg-gradient-to-br from-white to-sky-50 rounded-xl p-6 border border-sky-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-base text-gray-900 bg-white px-3 py-2 rounded-lg border">
                  {formData.cantidad} {unidadesMedida.find((u) => u.id === formData.unidadmedidaid)?.descripcion || ""}
                </p>
              </div>
  ...
  const renderStep4 = () => (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-gray-900 mb-2">¡Fórmula Actualizada Exitosamente!</h3>
      <p className="text-gray-500 mb-6">Los cambios han sido guardados correctamente en el sistema.</p>
      <div className="flex gap-4 justify-center">
        <Button onClick={() => router.push("/formulas")}>Ver Fórmulas</Button>
        <Button variant="outline" onClick={() => router.push(`/formulas/editar?getFormulaId=${formulaId}`)}>
          Continuar Editando
        </Button>
      </div>
    </div>
    ...
    return (
      <div className="container mx-auto py-6 px-4">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">No se proporcionó un ID de fórmula válido.</p>
          <Button onClick={() => router.push("/formulas")}>Volver a Fórmulas</Button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      <div className="mb-5">
        ...
        <p className="text-gray-600 mt-2">Modifica la información de la fórmula siguiendo los pasos del asistente</p>
      </div>
      ...
      <Card className="bg-white/80 backdrop-blur-sm border-slate-200/60 shadow-lg">
        <CardHeader className="bg-gradient-to-r from-slate-50/80 to-slate-100/50 backdrop-blur-sm">
          ...
          <CardDescription className="text-slate-600">Editando fórmula ID: {formulaId}</CardDescription>
        </CardHeader>
        ...
        <div className="flex justify-between mt-6">
          ...
          <div className="flex gap-2">
            {currentStep === 3 ? (
              ...
              >
                {isLoading ? "Finalizando..." : "Finalizar Edición"}
              </Button>
            ) : (
              <Button
